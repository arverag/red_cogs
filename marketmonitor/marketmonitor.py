...
class MMonitor:
    ...
    async def monitor(self, ctx, channel):
        ...
        This is cross-server. Type only the channel name or the ID that you want the reports to exit from."""
        ...
        while msg == "" or msg is not None:
            msg = await self.bot.wait_for_message(author=author,
                                                  channel=author_channel)
            if msg is not None and msg.content.lower() != "exit" and msg.content.lower() is in msgfilter:
                try:
